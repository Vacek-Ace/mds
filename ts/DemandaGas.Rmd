...
Se confirma visualmente las estacionalidades supuestas. Además de la tendencia creciente. Por tanto, parece que la serie no va a ser estacionaria en media. Como era de esperar.
...
No es posible identificar más estructura a partir del ACF y PACF, no hay problemas de estacionariedad o invertibilidad. Además, todos los coeficientes son significativos. Aunque algunas de las autocorrelaciones se siguen saliendo de las bandas. Procedemos al análisis de los residuos del modelo.
...
aug %>% features(.resid, ljung_box, lag=14, dof=4)
```

Los resultados no son significativos (es decir, el *p-valor* es relativamente grande). Por lo tanto, NO podemos concluir que los residuos no están correlados.
...
```{r}
nested_av_errors <- nested_cv(demandaGas, 7, last(demandaGas_train$date))
nested_av_errors
```
